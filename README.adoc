...
....
python ./mat73_to_pickle.py WM811K.mat
....

started at 13:45

We named the resulting file `WM811K.pkl`.
...
image::_images/Ref_Architecture.png[Reference Architecture, 640]

image::_images/Ref_Architecture_descriptions.png[Reference Architecture Descriptions, 480]
...
If you have not done so, clone this repo.

=== Create an S3 bucket

We will need an S3 bucket to hold the CloudFormation templates, you can either use an existing bucket (e.g. the one you used to store the data) or can create a new one.

Here is the command to create a new one:
...
Before you can create the CloudFormation stack, you will need to have the necessary CloudFormation parameters. Before you run the create.sh command, you should have these parameters defined:
...
Now run the `create.sh` command to launch the master stack and nested stacks:
