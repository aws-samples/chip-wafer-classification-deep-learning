...
2. Extend the ML environment initially created using IoT to do real-time detection.
...
As mentioned above, the source data is from the WM-811K data set (<<MIR_Lab>> and <<Qingyi>>).  Place this data into an S3 bucket organized into `train` and `valid` folders. The notebook `DataPrep.ipynb` documents the data preparation steps.
