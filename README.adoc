...
We reference several publicly available methods and sources, and we extend those by leveraging native AWS Machine Learning Services and IoT tools.
...
* AWS CLI is setup, see the https://aws.amazon.com/cli/[AWS CLI Docs] for more info
* An Amazon EC2 key pair
* RaspberryPi, optional for doing local inference
...
A wafer bin map shows a visual representation of failed dies across a wafer, and these have been put in categories or bins. The maps are created with probes and precise testing equipment. Equipment can fail entirely or fall out of tolerances. Both <<one>> and <<two>> discuss how wafer bin maps can be classified in to nine main categories:  Center, Donut, Edge-Loc, Edge-Ring, Loc, Random, Scratch, Near-full and none. __Figure 1__ is a visual representation of each.

.Nine wafer bin map classifications
[#wafter-classifications]
[caption="Figure 1: "]
image::_images/Nine_wafer_Classifications.png[Nine Wafer Classfications, 640]
{nbsp}
...
It is assumed that you have basic git knowledge and have an AWS Account. This workshop has two parts:

1. Use existing wafer data to build an ML Deep Learning model to classify and validate the model is working correctly.
2. Extend the previously launched ML environment using IoT to enable real-time defect detection.

=== Clone this repo

If you have not done so, clone this repo to your local machine (`git clone <repo-name>`).

=== AWS CLI

There are several steps that use the AWS CLI. If you have not installed and configured the AWS CLI, please do that now. Here is the documentation to that:  https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-install.html[Installing the AWS CLI]

To verify the AWS CLI is working, run this command and it should return a list of you S3 buckets:
...
You will need an S3 bucket for both the data set and the CloudFormation templates. You can use an existing bucket or you can create a new bucket. Here is an example command to create a bucket:

....
aws s3 mb s3://<myname>-wafer-workshop
....

=== Copy CloudFormation templates to an s3 bucket

The workshop uses an s3 bucket to store the CloudFormation templates used to launch the resources. Using the bucket you created above or an existing one, copy the templates to the bucket under a `templates` folder. For example, this assumes you are in the repo you have already cloned:

....
aws s3 sync cfn/ s3://duff-wafer-workshop/templates/
....

Make note of both the bucket and the folder, as these will used later to launch the stack(s).
...
- <<MIR_Lab>> Original source.

- <<Qingyi>>

=== Data Conversion

This workshop requires that the wafer data be in Python format, specifically we're using a pickle file. If you downloaded the data from the <<Qingyi>> link, the data is already a Python pickle object (.pkl) and you do not need to convert the data from MATLAB to Python.

We downloaded the wafer data from <<MIR_Lab>>, and the resulting unzipped file is a MATLAB 7.3 HDF5 file, as seen with the `head` command:

....
$ head WM811K.mat
MATLAB 7.3 MAT-file, Platform: PCWIN64, Created on: Sun Dec 29 20:48:11 2013 HDF5 schema ...
....

To convert this file we used the `mat73_to_pickle.py` command from the https://github.com/emanuele/convert_matlab73_hdf5[convert_matlab73_hdf5] repo (needs modules `numpy` and `h5py`):

=== Data placement in to an S3 bucket

As mentioned above, the source data is from the WM-811K data set (<<MIR_Lab>> and <<Qingyi>>).  Download this data set and place this data into an S3 bucket organized into `train` and `valid` folders. The notebook `DataPrep.ipynb` documents the data preparation steps.


== Reference Architecture

image::_images/Ref_Architecture.png[Reference Architecture, 800]

image::_images/Ref_Architecture_descriptions.png[Reference Architecture Descriptions, 640]
...
=== Launch the master stack

The CloudFormation stack that we create is a master or root stack that has several nested stacks, here are the file names for each:
....
master.yaml
  ├── app.yaml
  ├── iot.yaml
...
1. Launch the master stack by going to the: **https://console.aws.amazon.com/console[AWS Console] -> CloudFormation -> Create Stack -> Upload a template file -> Choose file**

2. Navigate to this repo that you cloned local to your system, and select **cfn/master.yaml**

3. You should see **Specify stack details**:
+
image::_images/cfn_1.png[Specify stack details, 680]
+
Fill in the parameters, here are descriptions of each:
+
|===
| *Parameter* | *Variable Name* | *Description*
| *Security Access CIDR*
| SecurityAccessCIDR
| CIDR block to limit inbound access for only SSH. Default: '0.0.0.0/0'

| *Project Tag*
| ProjectTag
| Tag to apply to created resources for visibility. Default: ChipWaferAnalysis
...
4. After you have entered values for the parameters, click **Next**.

5. Accept the default values of the **Configure stack options** and **Advanced options** sections and click **Next**. You can optionally set tags if desired.

6. Review the CloudFormation stack settings.

7. Click all checkboxes in the blue **Capabilities** box at the bottom of the page, as show here:
+
image::_images/cfn_2.png[Capabilities, 680]

8. Note the CodeCommit repos output from the stack, see the **Outputs** tab when the master stack is at __CREATE_COMPLETE__.

=== Do a first commit on CodeCommit repos

Check in the code from the `pytorch_code`, `test_code`, `deploy_code`, and `trainer_code` directories.
